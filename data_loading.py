...
class DataCore():
    ...
    def load_excel(self,path : str) -> None:
        ...
        self.excel_data = pd.read_excel(path,engine="openpyxl")

    def create_excel_template(self,n_connectors,path="template.xlsx",n_wavelengths=1,n_fibers=1):
        
        Sheet = np.zeros((n_connectors*n_fibers+2,n_connectors+2),dtype=object)
        
        #setting constant cells
        Sheet[0,0] = "Reference Configuration"
        Sheet[0,1] = "Reference Connector"
        Sheet[1,0] = "Fiber Number"
        Sheet[1,1] = "DUT"
        #setting numbering
        Sheet[1,2:n_connectors+2] = np.linspace(1,n_connectors,n_connectors)
        Sheet[2:n_fibers*n_connectors+2,1] = np.tile(np.linspace(1,n_connectors,n_connectors),n_fibers)
        Sheet[2:n_fibers*n_connectors+2,0] = np.repeat(np.linspace(1,n_fibers,n_fibers),n_connectors)
        ...
        for worksheet in workbook.worksheets():
            column_letter = xlsxwriter.utility.xl_col_to_name(n_connectors+1)

            worksheet.set_column("A:B",20)
            worksheet.merge_range(f"C1:{column_letter}1", "Connector Number",merge_format)
